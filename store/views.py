...
def checkout_paypal(request, cart, orders):
    if request.user.is_authenticated():
        ...
        paypalrestsdk.configure({
            "mode":"sandbox",
            "client_id":"AQiG40uvXcR7lWSqo2Gkn5VWX2nDUZmYLDMJJFoKD9fyvpRhKWm0-krKYKy6AwZp67UyOQsOwk2GUqXC",
            "client_secret":"ENNqlHjqcAonik3wbqBKtWduhNefvi_enc_sMW9I1uG0DM9RlQ7z5hNafu_4L9ULnwDLeoSyf2Lb-C0n",
        })
...
def checkout_stripe(cart, orders, token):
    stripe.api_key = "sk_test_OcrCNsnrBMvaODOoyonyNjGa"
    total=0
    for order in orders:
        total += (order.book.price * order.quantity)
    status = True
